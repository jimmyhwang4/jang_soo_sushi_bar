import React from "react";

function MenuContent() {
    return (
        <div className="menuContainer">

            <div className="menuRow">
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Appetizers</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Oyster (6 pieces)</p>
                            <p>Shrimp Tempura (M)</p>
                            <p>Shrimp Tempura (L)</p>
                            <p>Seafood Pancake</p>
                            <p>Corn Cheese</p>
                        </div>
                        <div className="menuColumn">
                            <p>$11.95</p>
                            <p>$15.95</p>
                            <p>$24.95</p>
                            <p>$18.95</p>
                            <p>$7.95</p>
                        </div>
                    </div>
                </div>
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Noodles</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Cold Soba Noodle</p>
                            <p>Shrimp Tempura Udon</p>
                            <p>Japanese Pot Noodle</p>
                            <p>Udon with Chicken</p>
                            <p>Udon with Beef</p>
                        </div>
                        <div className="menuColumn">
                            <p>$12.95</p>
                            <p>$14.95</p>
                            <p>$10.95</p>
                            <p>$13.95</p>
                            <p>$14.95</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuRow">
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Hot Soups</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Combination Fish Soup (Spicy)</p>
                            <p>Fish Roe Casserole</p>
                            <p>Codfish Soup (Mild)</p>
                            <p>Codfish Soup (Spicy)</p>
                            <p>Fish Intestine Soup</p>
                        </div>
                        <div className="menuColumn">
                            <p>$18.95</p>
                            <p>$18.95</p>
                            <p>$18.95</p>
                            <p>$18.95</p>
                            <p>$18.95</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuSingleRow">
                <div className="menuSingleSectionTitle">
                    <h2>Special Rolls (30% OFF!)</h2>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Crunch Dragon Roll</p>
                            <p>Caterpillar Roll</p>
                            <p>Las Vegas Roll</p>
                            <p>Baked Salmon Roll</p>
                            <p>Jumbo Volcano Roll</p>
                            <p>Hawaiian Roll</p>
                            <p>Spicy Tuna Tempura Roll</p>
                            <p>Three Amigos Roll</p>
                            <p>Playboy Roll</p>
                            <p>Crunch Roll</p>
                            <p>Rainbow Roll</p>
                            <p>Deluxe Eel Roll</p>
                            <p>Alaska Roll</p>
                            <p>Fire Cracker Roll</p>
                            <p>Ahi Roll</p>
                            <p>Tataki Roll</p>
                            <p>Double Shrimp Roll</p>
                        </div>
                        <div className="menuColumnDescription">
                            <p>spicy tuna, shrimp tempura, crab mix, eel sauce, tempura crunch</p>
                            <p>crab mix, eel, cucumber, avocado, eel sauce</p>
                            <p>deep fried salmon, crab mix, avocado, eel, eel sauce</p>
                            <p>baked salmon, california roll, eel sauce</p>
                            <p>baked albacore, california roll, eel sauce</p>
                            <p>shrimp tempura, crab mix, albacore, ponzu</p>
                            <p>deep fried spicy tuna, ponzu, eel sauce</p>
                            <p>crab mix, spicy tuna, tuna, vegetables, ginger sauce</p>
                            <p>shrimp tempura, spicy tuna, tuna, eel sauce, cream sauce</p>
                            <p>shrimp tempura, crab mix, cucumber, avocado, eel sauce</p>
                            <p>tuna, salmon, tai, shrimp, avocado, california roll</p>
                            <p>eel, avocado, california roll, eel sauce</p>
                            <p>salmon, california roll</p>
                            <p>salmon, spicy tuna roll</p>
                            <p>tuna, california roll</p>
                            <p>seared tuna, seared salmon, california roll, ponzu, hot sauce</p>
                            <p>sushi shrimp, crunch roll</p>
                        </div>
                        <div className="menuColumn">
                            <p>$̶1̶2̶.̶0̶0̶  $8.40</p>
                            <p>$̶1̶4̶.̶0̶0̶  $9.80</p>
                            <p>$̶1̶3̶.̶0̶0̶  $9.10</p>
                            <p>$̶1̶2̶.̶0̶0̶  $8.40</p>
                            <p>$̶1̶2̶.̶0̶0̶  $8.40</p>
                            <p>$̶1̶3̶.̶0̶0̶  $9.10</p>
                            <p>$̶1̶0̶.̶0̶0̶  $7.00</p>
                            <p>$̶1̶5̶.̶0̶0̶  $10.50</p>
                            <p>$̶1̶4̶.̶0̶0̶  $9.80</p>
                            <p>$̶1̶0̶.̶5̶0̶  $7.35</p>
                            <p>$̶1̶3̶.̶0̶0̶  $9.10</p>
                            <p>$̶1̶4̶.̶0̶0̶  $9.80</p>
                            <p>$̶1̶1̶.̶5̶0̶  $8.05</p>
                            <p>$̶1̶2̶.̶5̶0̶  $8.75</p>
                            <p>$̶1̶2̶.̶5̶0̶  $8.75</p>
                            <p>$̶1̶3̶.̶5̶0̶  $9.45</p>
                            <p>$̶1̶3̶.̶5̶0̶  $9.45</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuRow">
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Live Seafood</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Live Sweet Shrimp (Seasonal)</p>
                            <p>Live Octopus</p>
                        </div>
                        <div className="menuColumn">
                            <p>MP</p>
                            <p>MP</p>
                        </div>
                    </div>
                </div>
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Sushi Bar Special</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Omakase (per person)</p>
                        </div>
                        <div className="menuColumn">
                            <p>$90.00</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuRow">
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Sushi Combination</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>Small (11 pieces)</p>
                            <p>Medium (14 pieces)</p>
                            <p>Large (20 pieces)</p>
                        </div>
                        <div className="menuColumn">
                            <p>$20.95</p>
                            <p>$23.95</p>
                            <p>$36.95</p>
                        </div>
                    </div>
                </div>
                <div className="menuSectionTitle">
                    <div className="mobileTitle">
                        <h2>Roll Combination</h2>
                    </div>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <p>California Roll, Spicy Tuna Roll, & Crunch Roll</p>
                        </div>
                        <div className="menuColumn">
                            <p>$22.95</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuSingleRow">
                <div className="menuSingleSectionTitle">
                    <h2>Live Halibut Special</h2>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <div className="menuSpacing">
                                <p>Live Flounder</p>
                            </div>
                            <div className="menuSpacing">
                                <p>Small</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>Medium</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>Large</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>X-Large</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                        </div>
                        <div className="menuColumnDescription">
                            <div className="menuSpacing">
                                <p><br /></p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi salad, abalone, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi salad, abalone, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi salad, abalone, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi salad, abalone, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or Roll *</p>
                            </div>
                        </div>
                        <div className="menuColumn">
                            <div className="menuSpacing">
                                <p>$99.95</p>
                            </div>
                            <div className="menuSpacing">
                                <p>$115</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$145</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$170</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$200</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div className="menuSingleRow">
                <div className="menuSingleSectionTitle">
                    <h2>Sashimi Combo</h2>
                    <div className="menuSection">
                        <div className="menuColumn">
                            <div className="menuSpacing">
                                <p>Mini</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>Small</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>Medium</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>Large</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>X-Large</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                        </div>
                        <div className="menuColumnDescription">
                            <div className="menuSpacing">
                                <p>live halibut, sashimi combo, sashimi salad, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or California Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi combo, sashimi salad, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or California Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi combo, sashimi salad, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup, abalone, sea urchin</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or California Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>live halibut, sashimi combo, sashimi salad, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup, abalone, sea urchin, fatty tuna</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or California Roll *</p>
                            </div>
                            <div className="menuSpacing">
                                <p>½ live halibut, ½ sashimi combo, sashimi salad, seafood platter,</p>
                                <p>assorted tempura, spicy fish soup, two abalones, sea urchin, fatty tuna</p>
                                <p>* [ CHOOSE 1 ] Soju, Beer, or California Roll *</p>
                            </div>
                        </div>
                        <div className="menuColumn">
                            <div className="menuSpacing">
                                <p>$90</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$115</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$145</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$170</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                            <div className="menuSpacing">
                                <p>$200</p>
                                <div className="br">
                                    <p><br /></p>
                                    <p><br /></p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>
    );
}

export default MenuContent;